...
void MyCamera::SetPositionTargetAndUpward(vector3 a_v3Position, vector3 a_v3Target, vector3 a_v3Upward)
{
	...
	m_v3Position = a_v3Position;	//updating values to bring into calculate view
...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	quaternion quat;
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.x), vector3(1.0f, 0.0f, 0.f));
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.y), vector3(0.0f, 1.0f, 0.f));
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.z), vector3(0.0f, 0.0f, 1.f));
	//create our quaternion
	vector3 result = glm::cross(quat, vector3(0, 0, a_fDistance));	//multiply that by the vector representing movement
	m_v3Position += result;	//add the resulting vector to the existing values to update them
	m_v3Target += result;
...
{
	m_v3Position += vector3(0.0f, a_fDistance, 0.0f);	//q+e will always move straight up and down
	m_v3Target +=vector3(0.0f,a_fDistance,0.0f);

}
void MyCamera::MoveSideways(float a_fDistance)
{
	//same as in MoveForward
	quaternion quat;
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.x), vector3(1.0f, 0.0f, 0.f));
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.y), vector3(0.0f, 1.0f, 0.f));
	quat = quat * glm::angleAxis(glm::radians(m_v3PitchYawRoll.z), vector3(0.0f, 0.0f, 1.f));
	vector3 result = glm::cross(quat, vector3(a_fDistance, 0, 0));
	m_v3Position += result;
	m_v3Target += result;
}
void MyCamera::CalculateView(void)
{
	...
	m_v3Upward = glm::cross(quat, m_v3Upward);		//apply it to the position, target, and upward
	m_v3Position = glm::cross(quat, m_v3Position);
	m_v3Target = glm::cross(quat, m_v3Target);
	m_v3PitchYawRoll = ZERO_V3;		//reset these values so they don't continue to change the camera settings
	m_m4View = glm::lookAt(m_v3Position, m_v3Target, m_v3Upward);	//look at the updated position
}
